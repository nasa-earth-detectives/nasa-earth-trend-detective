# 📅 Sprints y Roadmap de Trabajo

Volver al [[00-Map-Of-Content]] | Ver roles en [[03-Roles-y-Equipo]]

---

## 🏃 Roadmap de los 4 Sprints

El proyecto divide el desarrollo en **4 Sprints técnicos y científicos**, con **20 tareas principales** y **55 subtareas atómicas**:

### 🔹 Sprint 1: Datos, ETL y Arquitectura Base
- **[S1-T1]** Configurar Solución .NET 10 Clean Architecture (6h) ➔ **July**
  - Subtareas: S1-T1.1 (Sln modular), S1-T1.2 (Swagger), S1-T1.3 (Excepciones RFC 7807)
- **[S1-T2]** Extractores Datasets Satelitales NASA (8h) ➔ **Fabriany (Reving)**
  - Subtareas: S1-T2.1 (EarthData Client), S1-T2.2 (Normalizador Grilla), S1-T2.3 (Limpieza Nulos)
- **[S1-T3]** Formulación matemática de Mann-Kendall (6h) ➔ **Johan**
  - Subtareas: S1-T3.1 (Especificación S y Var), S1-T3.2 (Series Control), S1-T3.3 (Cálculo Z)
- **[S1-T4]** Setup inicial React 19 + Globe.gl 3D (6h) ➔ **Diego**
  - Subtareas: S1-T4.1 (Vite + WebGL), S1-T4.2 (Textura Blue Marble), S1-T4.3 (OrbitControls)
- **[S1-T5]** Design System espacial Glassmorphism (5h) ➔ **Brayan**
  - Subtareas: S1-T5.1 (Tokens Tailwind), S1-T5.2 (Clases Blur 16px), S1-T5.3 (Navbar Espacial)

### 🔹 Sprint 2: Motor Científico y Pipeline Analítico ([[15-Trazabilidad-ClickUp-Sprint-2]])
- **[S2-T1]** Endpoints REST y DTOs tipados (8h) ➔ **July**
- **[S2-T2]** Almacén Columnar DuckDB embebido (8h) ➔ **Fabriany (Reving)**
- **[S2-T3]** Algoritmo de Mann-Kendall y Sen's Slope (8h) ➔ **Johan**
- **[S2-T4]** Hexágonos 3D y Mapas de Calor dinámicos (7h) ➔ **Diego**
- **[S2-T5]** Panel "Inspector de Detective" (5h) ➔ **Brayan** *(En Desarrollo Activo)*
  - Subtareas: S2-T5.1 (Tarjeta DetectiveCard Glassmorphism), S2-T5.2 (Badges Significancia \|Z\| > 1.96), S2-T5.3 (Gráfico SVG de Series Temporales)

### 🔹 Sprint 3: Experiencia 3D, Tendencias Opuestas e Integración
- **[S3-T1]** Seguridad de API, Rate Limiting y Caché (6h) ➔ **July**
- **[S3-T2]** Optimización de consultas DuckDB <50ms (6h) ➔ **Fabriany (Reving)**
- **[S3-T3]** Motor de Tendencias Opuestas (Opposing Trends) (8h) ➔ **Johan**
- **[S3-T4]** Anillos pulsantes (Ripples) y arcos 3D (7h) ➔ **Diego**
- **[S3-T5]** Componente Time-Slider interactivo 2000-2026 (6h) ➔ **Brayan**

### 🔹 Sprint 4: Pulido, Rigor Científico, QA y Pitch NASA
- **[S4-T1]** Pruebas de integración API y Swagger (5h) ➔ **July**
- **[S4-T2]** Seeders DuckDB para Demo Offline (4h) ➔ **Fabriany (Reving)**
- **[S4-T3]** Validación estadística cruzada contra NASA (6h) ➔ **Johan**
- **[S4-T4]** Optimización WebGL para 60 FPS estables (5h) ➔ **Diego**
- **[S4-T5]** Guion, Video Pitch y Despliegue Final NASA (8h) ➔ **Brayan**

---

## 🔗 Enlaces Relacionados
- [[05-Rigor-Cientifico-MannKendall]]
- [[06-Pipeline-CI-CD]]
